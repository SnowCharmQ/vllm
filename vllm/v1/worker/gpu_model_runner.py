...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def __init__(
        ...
        self.positions = torch.zeros(self.max_num_tokens,
                                     dtype=torch.int64,
                                     device=self.device)
        self.diff_emb = torch.zeros((self.max_num_tokens, 8, 1024),
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            self.requests[req_id] = CachedRequestState(
                ...
                diff_emb=new_req_data.diff_emb,
        ...
        for req_data in scheduler_output.scheduled_cached_reqs:
            ...
            req_state.diff_emb = req_data.diff_emb
    ...
    def _prepare_inputs(
        ...
        req_diff_embs = torch.zeros((num_reqs, 8, 1024),
                                        dtype=torch.bfloat16)
        ...
        for i, req_id in enumerate(self.input_batch.req_ids):
            ...
            max_num_scheduled_tokens = max(max_num_scheduled_tokens,
                                           num_tokens)
            req_diff_embs[i] = self.requests[req_id].diff_emb
        ...
        diff_emb_indices_tensor = torch.tensor(req_indices, dtype=torch.long)
        expanded_diff_emb_indices_tensor = torch.index_select(
            req_diff_embs,
            dim=0,
            index=diff_emb_indices_tensor
        )
        self.diff_emb[:total_num_scheduled_tokens].copy_(
            expanded_diff_emb_indices_tensor,
            non_blocking=True
        )
    ...
    def execute_model(
        ...
        if self.is_multimodal_model:
            ...
            diff_emb = self.diff_emb[:num_input_tokens]
            inputs_embeds = None
        if self.uses_mrope:
