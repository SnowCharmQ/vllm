...
class Qwen2ForCausalPersonalLM(Qwen2ForCausalLM):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.l_diff_token_ids = [151673 + i for i in range(8)]
        self.g_diff_token_ids = [151681 + i for i in range(8)]
        ...
        self.align_mlp_l_diff = nn.Sequential(
            nn.Linear(self.emb_hidden_size, self.config.hidden_size, dtype=torch.bfloat16),
            nn.GELU(),
            nn.Linear(self.config.hidden_size, self.config.hidden_size, dtype=torch.bfloat16),
        )
        self.align_mlp_g_diff = nn.Sequential(
            nn.Linear(self.emb_hidden_size, self.config.hidden_size, dtype=torch.bfloat16),
            nn.GELU(),
            nn.Linear(self.config.hidden_size, self.config.hidden_size, dtype=torch.bfloat16),
        )
    ...
    def forward(
        ...
        for i in range(len(input_ids)):
            if input_ids[i] in self.his_token_ids + self.l_diff_token_ids + self.g_diff_token_ids:
                flag = True
                break
